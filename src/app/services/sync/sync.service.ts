...
export class SyncService {
  ...
  constructor(
    ...
    this.platform.ready().then(async () => {
      ...
      this.events.subscribe('checklist:ready2sync', async (data: any) => {
  ...
  async startSync() {
    ...
    if (checklists) {
      for (const checklist of Object.entries(checklists)) {
        console.log(`[${this.title}#startSync] checklist #${checklist[0]} (${checklist[1]['checklistType']})`, checklist[1]);
        ...
        await this.db.insertTbLog({
          ...
          plate: checklist[1]['str_placa'],
          schedule: syncStart,
          checklistType: checklist[1]['checklistType'],
        ...
        try {
          ...
          if (checklist[1]['syncFields'] == 0) {
            ...
            if (!newUploadChecklistResult) {
              await this.db.insertTbLog({
                ...
                status: 'Erro',
                plate: checklist[1]['str_placa'],
                schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
                checklistType: checklist[1]['checklistType'],
                message: 'newUploadChecklistResult = ' + newUploadChecklistResult
              }, this.title);
            ...
            if (!updateSyncFieldsResult) {
              await this.db.insertTbLog({
                ...
                schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
                checklistType: checklist[1]['checklistType'],
                message: 'updateSyncFieldsResult = ' + updateSyncFieldsResult
              }, this.title);
          ...
          if (checklist[1]['syncImages'] == 0) {
            ...
            if (!uploadChecklistMediaResult) {
              await this.db.insertTbLog({
                ...
                schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
                checklistType: checklist[1]['checklistType'],
                message: 'uploadChecklistMediaResult = ' + uploadChecklistMediaResult
              }, this.title);
            ...
            if (!updateSyncImagesResult) {
              await this.db.insertTbLog({
                ...
                schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
                checklistType: checklist[1]['checklistType'],
                message: 'updateSyncImagesResult = ' + updateSyncImagesResult
              }, this.title);
          ...
          await this.db.insertTbLog({
            ...
            schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
            checklistType: checklist[1]['checklistType'],
            message: 'Sincronização interrompida | ' + JSON.stringify(error)
          }, this.title);
          ...
          if (!updateSyncAttemptsResult) {
            await this.db.insertTbLog({
              ...
              schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
              checklistType: checklist[1]['checklistType'],
              message: 'updateSyncAttemptsResult = ' + updateSyncAttemptsResult
            }, this.title);
          }
  ...
  async finishChecklistSync(checklist: any, syncStart: any, startMS: any, postId: any) {
    ...
    if (countSyncFields == countSyncedFields) {
      ...
      await this.db.insertTbLog({
        ...
        schedule: syncFinish,
        checklistType: checklist[1].checklistType,
        message: `Sincronização concluída (${syncTime}) @${checklist[1].id}`
      }, this.title);
    } else {
      await this.db.insertTbLog({
        id_schedule: postId,
        ...
        schedule: syncFinish,
        checklistType: checklist[1].checklistType,
        message: `Sincronização incompleta @${checklist[1].id}`
      }, this.title);
    }
  }
  ...
  async newUploadChecklist(checklist: any, postId: any) {
    ...
    console.log(`[${this.title}#newUploadChecklist] checklistType`, checklist.checklistType);

    let url, headers, body;
    if (checklist.checklistType == 'recepcao') {
      ...
      body = {
        ...
        str_tipo: checklist.checklistType || '',
    ...
    await this.http.post(url, body, headers)
      .then(async response => {
        ...
        // } else if (checklist.checklistType != 'pagamento') {
        } else {
          //* update int_id_recepcao
      ...
      .catch(async error => {
        ...
        await this.db.insertTbLog({
          ...
          schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
          checklistType: checklist.checklistType,
          message: 'campos | ' + JSON.stringify(error)
        }, this.title);
  ...
  async uploadChecklistMedia(bodyArray: any, mediaArray: any): Promise<any> {
    ...
    await this.db.insertTbLog({
      id_schedule: bodyArray.int_id_recepcao,
      status: 'Sincronizando',
      plate: bodyArray.str_placa,
      schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
      checklistType: bodyArray.checklistType,
      message: 'imagens'
    }, this.title);
    ...
    for (const [key, value] of Object.entries(mediaArray)) {
      try {
        ...
        const postFilename = `${bodyArray.int_id_recepcao}-${bodyArray.checklistType}-${key}.png`;
        ...
        body.append('str_tipo', bodyArray.checklistType);
        body.append('str_descricao', bodyArray.checklistType);
        ...
        await this.db.insertTbLog({
          ...
          schedule: new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }),
          checklistType: bodyArray.checklistType,
          message: 'imagens - ' + key + ' | ' + JSON.stringify(error)
        }, this.title);
